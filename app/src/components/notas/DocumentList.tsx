...
function PrescriptionBadge({ status }: { status: 'borrador' | 'firmada' }) {
  if (status === 'firmada')
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-sky-100 text-sky-800 dark:bg-sky-900/40 dark:text-sky-300">
...
function ConsentBadge({ status }: { status: ConsentForm['status'] }) {
  const map = {
    ...
    'firmado-presencial': 'bg-sky-100 text-sky-800 dark:bg-sky-900/40 dark:text-sky-300',
...
function SectionHeader({
  icon, title, count, accentClass, open, onToggle, onNew, newLabel,
}: {
  icon: React.ReactNode; title: string; count: number; accentClass: string
  open: boolean; onToggle: () => void; onNew?: () => void; newLabel: string
}) {
  return (
    <div className="flex items-center justify-between px-4 py-3">
      <button onClick={onToggle} className="flex items-center gap-3 flex-1 text-left group">
        <span className={`flex items-center justify-center w-8 h-8 rounded-lg ${accentClass} text-white shadow-sm`}>
          {icon}
        </span>
        <span className="font-semibold text-slate-800 dark:text-slate-100 group-hover:text-sky-700 dark:group-hover:text-sky-400 transition-colors">
          {title}
        </span>
        ...
        <svg className={`ml-auto w-4 h-4 text-slate-400 transition-transform ${open ? 'rotate-180' : ''}`}
      ...
      {onNew && (
        <button onClick={onNew}
          className="ml-3 flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-sky-600 hover:bg-sky-700 text-white transition-colors shadow-sm">
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-10 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-slate-900 dark:text-white truncate">{currentPatient.name}</h1>
            <p className="text-xs font-mono text-slate-400 dark:text-slate-500">{currentPatient.expedienteNumber}</p>
          </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-3 pb-8">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
          <SectionHeader
            icon={<svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2" /><rect x="9" y="3" width="6" height="4" rx="1" /><path d="M9 12h6M9 16h4" /></svg>}
            title="Notas de Evolución" count={patientNotes.length} accentClass="bg-violet-500"
            open={openSections.evolution} onToggle={() => toggle('evolution')}
            onNew={onNewNote} newLabel="Nueva nota"
          />
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
          <SectionHeader
            icon={<svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2" /><rect x="9" y="3" width="6" height="4" rx="1" /><path d="M9 12h6M9 16h4" /></svg>}
            title="Recetas Médicas" count={patientRx.length} accentClass="bg-sky-600"
            open={openSections.prescriptions} onToggle={() => toggle('prescriptions')}
            onNew={onNewPrescription} newLabel="Nueva receta"
          />
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
          <SectionHeader
            icon={<svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><path d="M14 2v6h6" /><path d="M16 13H8M16 17H8M10 9H8" /></svg>}
            title="Consentimientos Informados" count={patientConsents.length} accentClass="bg-sky-600"
            open={openSections.consents} onToggle={() => toggle('consents')}
            onNew={onNewConsent} newLabel="Nuevo consentimiento"
          />
