...
import type { ConsentFormDetailProps } from '../types'
...
export function ConsentFormDetail({
  ...
  function handleSignPresential() {
    ...
    const canvas = canvasRef.current!
    onSignPresential?.(consent.id, canvas.toDataURL('image/png'))
  }
  ...
  const statusMap = {
    ...
    'firmado-presencial': { label: 'Firmado presencialmente', cls: 'bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-700/50' },
                ...
                {emailSent && (
                  <div className="flex items-center gap-2 px-4 py-3 rounded-lg bg-sky-50 dark:bg-sky-900/20 border border-sky-100 dark:border-sky-800/50">
                    ...
                    <p className="text-xs text-sky-700 dark:text-sky-400">
                      ...
                      {consent.emailSentAt && (
                        <span className="text-sky-500 dark:text-sky-500 ml-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        ...
                        className="flex flex-col items-center gap-2 px-4 py-4 rounded-xl border-2 border-slate-200 dark:border-slate-600 hover:border-teal-400 dark:hover:border-teal-500 hover:bg-teal-50 dark:hover:bg-teal-900/20 transition-all group"
                      >
                        <svg className="w-6 h-6 text-slate-400 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8}>
                          <rect x="2" y="3" width="20" height="14" rx="2" />
                          <path d="M8 21h8M12 17v4" />
                        </svg>
                        <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 group-hover:text-teal-700 dark:group-hover:text-teal-400 transition-colors text-center">
                          Firma presencial
                        </span>
