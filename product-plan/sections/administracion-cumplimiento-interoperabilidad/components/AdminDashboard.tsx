...
const roleColors: Record<UserRole, string> = {
  medico: 'bg-teal-100 text-teal-800 dark:bg-teal-900/40 dark:text-teal-300',
...
const actionColors: Record<AuditAction, string> = {
  ...
  creacion: 'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-400',
...
const arcoStatusColors: Record<ArcoStatus, string> = {
  ...
  resuelta: 'bg-teal-100 text-teal-800 dark:bg-teal-900/40 dark:text-teal-300',
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-sm">
        ...
        <div className="px-5 py-4 border-t border-slate-100 dark:border-slate-700 flex gap-2">
          ...
          <button
            ...
            className="flex-1 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold transition-colors"
          >
            Guardar
          </button>
        </div>
...
function DashboardTab({ metrics, complianceBadges }: Pick<AdminDashboardProps, 'metrics' | 'complianceBadges'>) {
  const metricCards = [
    ...
    },
    {
      ...
      color: metrics.pendingArcoRequests > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-teal-600 dark:text-teal-400',
      bg: metrics.pendingArcoRequests > 0 ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-teal-50 dark:bg-teal-900/20',
    },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm flex items-center gap-3">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${metrics.encryptionActive ? 'bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400' : 'bg-rose-50 dark:bg-rose-900/20 text-rose-500'}`}>
          ...
          <div>
            ...
            <p className={`text-xs ${metrics.encryptionActive ? 'text-teal-600 dark:text-teal-400' : 'text-rose-500'}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {complianceBadges.map(badge => (
            <div key={badge.id} className="flex flex-col items-center gap-2 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700 text-center">
              <div className="w-8 h-8 rounded-full bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400 flex items-center justify-center">
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700">
          {users.map(user => (
            <div key={user.id} className="flex items-center gap-4 px-5 py-4">
              <div className="w-9 h-9 rounded-full bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-400 text-xs font-bold flex items-center justify-center flex-shrink-0">
                {initials(user.name)}
              </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {user.status === 'activo' ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-teal-50 dark:bg-teal-900/20 hover:bg-teal-100 dark:hover:bg-teal-900/40 text-teal-600 dark:text-teal-400 transition-colors"
                  >
                    Activar
                  </button>
                )}
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap">
        <select
          ...
          className="text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500"
        >
        ...
        {(filterUser || filterAction) && (
          <button onClick={() => { setFilterUser(''); setFilterAction('') }} className="text-xs text-teal-600 dark:text-teal-400 px-2 hover:underline">
            Limpiar filtros
          </button>
        )}
      </div>
...
function FhirTab({
  ...
  const statusColors = {
    completado: 'text-teal-600 dark:text-teal-400',
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-7 h-7 rounded-lg bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400 flex items-center justify-center">
          ...
          <div className="space-y-3">
            <input
              ...
              className="w-full text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-700 dark:text-slate-300 px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-teal-500 placeholder-slate-400"
            />
            <button
              onClick={() => { if (patientId) onExportFhirIndividual?.(patientId) }}
              disabled={!patientId}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-semibold bg-teal-600 hover:bg-teal-700 disabled:bg-slate-200 dark:disabled:bg-slate-700 disabled:text-slate-400 dark:disabled:text-slate-500 text-white transition-colors"
            >
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-700 dark:text-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
              <div>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700">
          {exports.map(exp => (
            <div key={exp.id} className="flex items-center gap-3 px-5 py-3">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${exp.type === 'individual' ? 'bg-teal-50 dark:bg-teal-900/20 text-teal-500' : 'bg-violet-50 dark:bg-violet-900/20 text-violet-500'}`}>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
          ...
          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${arcoRequests.filter(r => r.status === 'pendiente').length > 0 ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400' : 'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-400'}`}>
                      ...
                      <div className="flex flex-col gap-2 min-w-[160px]">
                        ...
                        <div className="flex gap-1">
                          ...
                          <button
                            ...
                            className="flex-1 py-1.5 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-[10px] font-semibold transition-colors"
                          >
                            Guardar
                          </button>
                        </div>
