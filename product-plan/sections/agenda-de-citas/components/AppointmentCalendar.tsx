...
const STATUS: Record<AppointmentStatus, StatusCfg> = {
  ...
  confirmada: {
    ...
    dot: 'bg-teal-500',
    pillBg: 'bg-teal-100 dark:bg-teal-900/40',
...
function DetailPanel({
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      {/* Hero strip */}
      <div className="bg-teal-600 dark:bg-teal-700 px-8 py-6">
      ...
      <div className="px-8 py-6 max-w-lg space-y-4">
        ...
        <InfoCard>
          <InfoRow
            icon={<Calendar className="w-4 h-4 text-teal-600 dark:text-teal-400" strokeWidth={1.5} />}
          ...
          <Divider />
          <InfoRow
            icon={<Clock className="w-4 h-4 text-teal-600 dark:text-teal-400" strokeWidth={1.5} />}
            iconBg="bg-teal-50 dark:bg-teal-950/40"
            label="Hora"
        ...
        <div className="space-y-2 pt-1">
          {apt.status === 'pendiente' && (
            <div className="flex gap-2">
              <button
                onClick={onConfirm}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-teal-600 hover:bg-teal-700 active:bg-teal-800 text-white text-sm font-semibold rounded-xl transition-colors shadow-sm"
              >
      ...
      <aside className="w-full md:w-[400px] lg:w-[460px] shrink-0 flex flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-800 space-y-4">
          <div className="flex items-start justify-between">
            ...
            <button
              onClick={onCreate}
              className="flex items-center gap-1.5 px-3 py-2 bg-teal-600 hover:bg-teal-700 active:bg-teal-800 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm shrink-0"
            >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-500 transition"
            />
            {searchQuery && (
              <button
              ...
              <div key={date}>
                ...
                <div
                  className={`flex items-center gap-3 px-6 py-2 ${
                    isToday(date)
                      ? 'sticky top-0 z-10 bg-teal-50 dark:bg-teal-950/30'
                      : ''
                  }`}
                >
                  <span
                ...
                {dayApts.map((apt) => {
                  ...
                  return (
                    ...
                    >
                      {/* Selected indicator */}
                      {isSelected && (
                        <div className="absolute left-0 top-2 bottom-2 w-0.5 bg-teal-500 rounded-r" />
                      )}
