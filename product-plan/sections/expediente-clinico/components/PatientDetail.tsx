// Fonts: DM Sans (headings), Inter (body), IBM Plex Mono (mono/codes)
// Colors: teal (primary), sky (secondary), slate (neutral)
// Design: clinical precision — structured sections, monospaced CIE-10 codes,
//         metric-card vitals, role-aware access indicator

import { useState } from 'react'
import type { PatientDetailProps, Patient, UserRole, DiagnosisStatus } from '../types'
import {
  ChevronDown,
  ArrowLeft,
  Pencil,
...
const DX_STATUS: Record<DiagnosisStatus, { label: string; bg: string; text: string }> = {
  activo: { label: 'Activo', bg: 'bg-teal-100 dark:bg-teal-900/40', text: 'text-teal-700 dark:text-teal-300' },
  ...
  return (
    <div
      className={`rounded-xl p-3 flex items-start gap-2.5 ${
        highlight
          ? 'bg-teal-50 dark:bg-teal-950/30 border border-teal-100 dark:border-teal-900'
          : 'bg-slate-50 dark:bg-slate-800/50'
      }`}
    >
      <div className={`shrink-0 mt-0.5 ${highlight ? 'text-teal-600 dark:text-teal-400' : 'text-slate-400 dark:text-slate-500'}`}>
        {icon}
      </div>
      <div>
        <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider">{label}</p>
        <p className={`text-sm font-bold font-mono mt-0.5 ${highlight ? 'text-teal-700 dark:text-teal-300' : 'text-slate-800 dark:text-slate-200'}`}>
          {value}
        </p>
      </div>
    </div>
...
function RoleBadge({ role }: { role: UserRole }) {
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${role === 'medico' ? 'bg-teal-500' : 'bg-sky-400'}`}
      />
...
export function PatientDetail({ patient, currentUserRole, onEdit, onBack }: PatientDetailProps) {
  const p = patient
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 flex-wrap">
                ...
                {isMedico && (
                  <button
                    onClick={() => onEdit?.(p.id)}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold rounded-lg transition-colors shrink-0"
                  >
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-3">
        ...
        {/* ── 1. Datos Generales ── */}
        <Section
          icon={<User className="w-4 h-4 text-teal-600 dark:text-teal-400" strokeWidth={1.5} />}
        ...
        {/* ── 4. Exploración Física ORL ── */}
        <Section
          icon={<Stethoscope className="w-4 h-4 text-teal-600 dark:text-teal-400" strokeWidth={1.5} />}
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                <VitalCard
                  icon={<HeartPulse className="w-4 h-4" strokeWidth={1.5} />}
                  label="T/A"
                  value={p.physicalExam.vitalSigns.bloodPressure}
                  highlight
                />
                <VitalCard
                  icon={<Activity className="w-4 h-4" strokeWidth={1.5} />}
                  label="Frec. cardíaca"
                  value={p.physicalExam.vitalSigns.heartRate}
                />
                <VitalCard
                  icon={<Thermometer className="w-4 h-4" strokeWidth={1.5} />}
                  label="Temperatura"
                  value={p.physicalExam.vitalSigns.temperature}
                />
                <VitalCard
                  icon={<Weight className="w-4 h-4" strokeWidth={1.5} />}
                  label="Peso"
                  value={p.physicalExam.vitalSigns.weight}
                />
                <VitalCard
                  icon={<Ruler className="w-4 h-4" strokeWidth={1.5} />}
                  label="Talla"
                  value={p.physicalExam.vitalSigns.height}
                />
                <VitalCard
                  icon={<Hash className="w-4 h-4" strokeWidth={1.5} />}
                  label="IMC"
                  value={p.physicalExam.vitalSigns.bmi}
                />
              </div>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start gap-3 px-4 py-3 bg-slate-50 dark:bg-slate-800/50">
                    <span className="font-mono text-xs font-bold text-teal-700 dark:text-teal-400 bg-teal-100 dark:bg-teal-900/40 px-2 py-1 rounded-md shrink-0">
                      {dx.code}
                    </span>
                    <div className="flex-1 min-w-0">
