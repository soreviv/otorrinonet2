...
- **Sitio público** (`src/app/(public)/`) — marketing, agendado de citas, tienda de productos médicos.
- **Panel interno** (`src/app/staff/`) — agenda, expediente clínico (EHR), recetas, consentimientos. Requiere sesión + 2FA.

La aplicación Next.js vive en la raíz del repositorio. Todos los comandos npm se ejecutan desde ahí.
...
**Archivos a modificar:** `prisma/schema.prisma`, `src/app/staff/pacientes/` (formulario)
...
**Archivo a crear:** `src/__tests__/schemas/tienda.test.ts`
**Archivo fuente:** `src/lib/schemas/tienda.ts`
...
**Archivo a crear:** `src/__tests__/lib/mailer.test.ts`
**Archivo fuente:** `src/lib/mailer.ts`
...
**Archivo a crear:** `src/__tests__/hooks/useCarrito.test.ts`
**Archivo fuente:** `src/hooks/useCarrito.ts`
...
| Archivo | Propósito |
|---------|-----------|
| `src/lib/dal.ts` | `verifySession()` — autenticación |
| `src/lib/mailer.ts` | Emails transaccionales + función `esc()` |
| `src/lib/schemas/tienda.ts` | Schemas Zod (tienda y checkout) |
| `src/hooks/useCarrito.ts` | Hook de carrito (localStorage) |
| `src/app/actions/appointments.ts` | Server Action de agendado |
...
- No modificar **bajo ninguna circunstancia** los archivos protegidos de Claude Code:
  ...
  - `next.config.ts`
  - `package.json` / `package-lock.json` (salvo que la tarea lo pida explícitamente)
